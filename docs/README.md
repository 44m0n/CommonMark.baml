...
The supported v1 surface is `parse`, `render_html`, and `to_html`, plus the AST types and the dialect types `ParseOptions`, `BlockStart`, and `InlineParseRule`. BAML cannot hide namespaces, so `Parser`, `Open`, scanners, spec helpers, and other `ns_*` functions are visible but unsupported: they may change without a major version. The project is pinned and tested on BAML nightly 0.19.1-nightly.20260911.a and passes 652/652 CommonMark examples.
