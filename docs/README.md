...
The supported v1 surface is `parse`, `render_html`, and `to_html`, plus the AST types and the dialect types `ParseOptions`, `BlockStart`, and `InlineParseRule`. BAML cannot hide namespaces, so `Parser`, `Open`, scanners, spec helpers, and other `ns_*` functions are visible but unsupported: they may change without a major version. The project is tested on BAML toolchain 0.19.0 (`baml toolchain use 0.19.0`) and passes 652/652 CommonMark examples.
